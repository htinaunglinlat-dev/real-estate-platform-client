...
export function PropertyTypeSelect({ value, onValueChange }: PropertyTypeSelectProps) {
  ...
  return (
    <div className="grid min-w-0">
      <label htmlFor={id} className="sr-only">Property Type</label>
      <NativeSelect id={id} name="property_type" value={value ?? ""}
        ...
        <NativeSelectOption value="">All property types</NativeSelectOption>
        {options.map((option) => (
          <NativeSelectOption key={option} value={option}>{option[0] + option.slice(1).toLowerCase()}</NativeSelectOption>
        ))}
      </NativeSelect>
    </div>
  );
