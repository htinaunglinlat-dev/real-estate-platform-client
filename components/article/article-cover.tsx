...
export function ArticleCover({ category, className }: { category: string; className?: string }) {
  const Icon = category === "Land Types" ? Layers3 : category === "Buying Guide" ? FileCheck2 : category === "Site Visits" ? Compass : Sprout;

  return (
    <div aria-hidden="true" className={cn("relative flex aspect-[16/9] items-center justify-center overflow-hidden bg-secondary text-primary", className)}>
      ...
      <span className="absolute bottom-4 left-5 text-[10px] font-semibold uppercase tracking-[0.2em]">Haven Journal</span>
    </div>
  );
}
